...
| `QualityPolicy` | model | `version`, `min_valid_fraction` (0.20, over `AnalysisMask.water_pixels`), `min_grid_coverage`, `cloud_dilation_px`, `aot550_max`, `allowed_aerosol_models`, `max_negative_rhos_fraction`, `max_residual_swir_rhow` (V8; the glint angle is recorded, not gated), `range_rules: tuple[RangeRule]`, `reject_on_ancillary_fallback` | DA-2. Numeric values are fixed in Phase 3 from spike data and reviewed at the DA-6 gate |
