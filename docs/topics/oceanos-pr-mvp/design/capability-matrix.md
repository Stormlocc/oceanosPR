...
| C3 | **Environment preflight** | release tag, commit SHA, LUTs, GSHHG, credentials, disk budget | No run starts against an ACOLITE whose version or commit differs from the pin. | none | `acolite` | new |
...
| C9 | **Coastal analysis mask** | GSHHG coastline, coastal buffer, analysis mask | Built once per grid. It excludes pixels from *statistics* only. The published layers keep the coastline (Q8). | `data/products/<aoi>/grid/` | `processing.masks` | new |
